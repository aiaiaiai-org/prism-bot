...
`AuthorizedUpdate` delegates `surface_context` and `reply_target` without changing the Hub-resolved human actor. A channel post, a message sent on behalf of a chat (`sender_chat`), or a bot sender never supplies human identity evidence. Channel management is currently unsupported. `/start` and `/context` in a channel receive only this static notice, with no Hub identity lookup or command execution:

> Керування Prism у каналі поки не підтримується. Відкрий особистий чат із ботом або гілку групи.

The notice is the only reply the bot can produce before any authorization, so it is sent only when `PRISM_BOT_ALLOWED_CHAT_IDS` enumerates the chats this instance serves and the channel is one of them. An unrestricted policy vouches for nobody: an operator who has not listed their chats would otherwise let any channel the bot administers drive unbounded `sendMessage` calls out of the shared Telegram rate budget by repeating `/start`. Under an unrestricted policy a channel therefore receives nothing.
...
The shared composition adds `/context`; `/start` prepends the same card to onboarding copy. Both remain subject to existing actor authorization and lifecycle gates in interactive chats. The card shows Prism, the chat title or surface label, surface type, chat ID, optional topic ID, and `Прив’язка Hub: не перевірена`. Rendering is bounded plain text; chat titles cannot inject new lines or Telegram markup. Title sanitisation covers Unicode separators such as U+2028 and U+2029, not only C0 control characters, because Telegram clients render those as line breaks and a group admin could otherwise forge a card line.
